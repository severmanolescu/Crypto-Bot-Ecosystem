"""
CryptoRSIHandler: A Python module for calculating RSI (Relative Strength Index)
...
class CryptoRSIHandler:
    """
    CryptoRSIHandler: A class for calculating RSI (Relative Strength Index)
    """
    ...
    def calculate_rsi_for_timeframes_parallel(
        ...
        # Find overbought/oversold pairs
        overbought_pairs = [
            (s, rsi) for s, rsi in rsi_values.items() if rsi > overbought_threshold
        ]
        oversold_pairs = [
            (s, rsi) for s, rsi in rsi_values.items() if rsi < oversold_threshold
        ]
