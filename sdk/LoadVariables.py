...
def load(file_path="ConfigurationFiles\\variables.json"):
    ...
    if not os.path.exists(file_path):
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Variables file '{file_path}' not found. Using default values.")
    ...
    try:
        ...
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Invalid JSON in variables file '{file_path}'. Using default values.")
        print(f"❌ Invalid JSON in variables file '{file_path}'. Using default values.")
        return {}
    except Exception as e:
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Error loading variables from '{file_path}': {e}. Using default values.")
...
def get_int_variable(var_name, default=1800, file_path="ConfigurationFiles\\variables.json"):
    ...
    if isinstance(value, int):
        ...
        try:
            ...
            logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Warning: '{var_name}' has an invalid integer format ('{value}'). Using default {default}.")
            print(f"⚠️ Warning: '{var_name}' has an invalid integer format ('{value}'). Using default {default}.")
            return default
    else:
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Warning: '{var_name}' is not a valid type. Using default {default}.")
...
def load_keyword_list(file_path="ConfigurationFiles\\keywords.json"):
    ...
    """
    if not os.path.exists(file_path):
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} File '{file_path}' not found. Returning an empty list.")
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as file:
            ...
            if isinstance(keywords, list):
                ...
                logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Warning: Expected a list in '{file_path}', but got something else. Returning empty list.")
                print(f"⚠️ Warning: Expected a list in '{file_path}', but got something else. Returning empty list.")
                return []
    except json.JSONDecodeError:
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Invalid JSON in '{file_path}'. Returning empty list.")
        print(f"❌ Invalid JSON in '{file_path}'. Returning empty list.")
        return []
    except Exception as e:
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Error loading '{file_path}': {e}. Returning empty list.")
...
def load_symbol_to_id(file_path="ConfigurationFiles\\symbol_to_id.json"):
    ...
    """
    if not os.path.exists(file_path):
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Symbol-to-ID file '{file_path}' not found. Using an empty mapping.")
    ...
    try:
        ...
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Invalid JSON in symbol-to-ID file '{file_path}'. Using an empty mapping.")
        print(f"❌ Invalid JSON in symbol-to-ID file '{file_path}'. Using an empty mapping.")
        return {}
    except Exception as e:
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Error loading symbol-to-ID mapping from '{file_path}': {e}. Using an empty mapping.")
...
def load_keywords(file_path="ConfigurationFiles\\keywords.json"):
    ...
    """
    if not os.path.exists(file_path):
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Keywords file '{file_path}' not found. Using an empty list.")
    ...
    try:
        ...
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Invalid JSON in keywords file '{file_path}'. Using an empty list.")
        print(f"❌ Invalid JSON in keywords file '{file_path}'. Using an empty list.")
        return []
    except Exception as e:
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Error loading keywords from '{file_path}': {e}. Using an empty list.")
...
def save_keywords(keywords, file_path="ConfigurationFiles\\keywords.json"):
    ...
    try:
        ...
        logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Error saving keywords to '{file_path}': {e}.")
