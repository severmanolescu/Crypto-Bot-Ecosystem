    ...
    async def handle_buttons(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if text == "🚨 Check for 1h Alerts" or text.lower() == "1h":
            ...
            if alert_available is False:
                await update.message.reply_text("😔 No major price movement for 1h timeframe")
            ...
            if alert_available is False:
                await update.message.reply_text("😔 No major price movement for 24h timeframe")

        elif text == "🚨 Check for 7d Alerts" or text.lower() == "7d":
            await update.message.reply_text("🚨 Searching for new alerts for 7d update...")

            alert_available = await self.start_the_alerts_check_7d(update)

            if alert_available is False:
                await update.message.reply_text("😔 No major price movement for 7d timeframe")
            ...
            if alert_available is False:
                await update.message.reply_text("😔 No major price movement for 30d timeframe")

        elif text == "🚨 Check for all timeframes Alerts" or text.lower() == "all":
            await update.message.reply_text("🚨 Searching for new alerts for all timeframes...")
